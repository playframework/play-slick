...
object TestData {
  ...
  val badConfiguration: Configuration = Configuration.from(
    Map("slick.dbs.missing-slick-profile.db.driver" -> "play.api.db.slick.SomeDummyDriver")).withFallback(configuration)

...
class Application @Inject() (catDao: CatDAO, dogDao: DogDAO, controllerComponents: ControllerComponents)(implicit
  ...
  val catForm = Form(mapping("name" -> text(), "color" -> text())(Cat.apply)(Cat.unapply))

  val dogForm = Form(mapping("name" -> text(), "color" -> text())(Dog.apply)(Dog.unapply))
